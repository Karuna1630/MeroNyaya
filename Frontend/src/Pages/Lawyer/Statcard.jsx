const colorMap = {
  ...
  slate: {
    bg: 'bg-linear-to-br from-slate-600 to-slate-800',
    light: 'bg-slate-400/20',
    ring: 'ring-slate-500/20',
  },
};

const StatCard = ({ icon, title, value, subtitle, className = "", onClick, color = "blue" }) => {
  ...
  return (
    <Wrapper
      ...
      onClick={onClick}
      className={`relative overflow-hidden rounded-2xl p-5 text-white shadow-lg ${c.bg} ring-1 ${c.ring} ${className}`.trim()}
    >
