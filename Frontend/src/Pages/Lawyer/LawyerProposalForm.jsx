...
const LawyerProposalForm = ({ isOpen, onClose, caseData, onSubmit, isSubmitting = false, error = null }) => {
  ...
  const getUrgencyBadge = (urgency) => {
    const badges = {
      High: { bg: "bg-red-500 text-white", text: "text-white", label: t('lawyerCaseRequest.high') },
      Medium: { bg: "bg-amber-100", text: "text-amber-700", label: t('lawyerCaseRequest.medium') },
      Low: { bg: "bg-blue-100", text: "text-blue-700", label: t('lawyerCaseRequest.low') },
    };
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all">
        ...
        <div className="p-8 pb-4 sticky top-0 bg-white z-10 border-b border-gray-50">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">{t('lawyerProposal.caseDetails')}</h2>
              <p className="text-gray-500 text-sm mt-1">{t('lawyerProposal.reviewTheCase')}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
            ...
            <div>
              <p className="font-medium text-gray-900 text-sm">{caseData.client_name || t('common.error')}</p>
              <p className="text-xs text-gray-500 mt-0.5">{t('lawyerProposal.caseBy')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 pt-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-gray-200 pt-6">
            <div>
              <p className="text-xs text-gray-500 uppercase mb-1">{t('cases.category')}</p>
              <p className="text-sm text-gray-900">
                {caseData.case_category || t('cases.allCategories')}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase mb-1">{t('lawyerFindCases.createdDate')}</p>
              <p className="text-sm text-gray-900">
          ...
          <div className="space-y-2 border-t border-gray-200 pt-6">
            <label className="text-sm font-medium text-gray-900">{t('lawyerProposal.proposalText')}</label>
            <textarea
              value={proposal}
              onChange={(e) => setProposal(e.target.value)}
              placeholder={t('lawyerProposal.writingProposal')}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            >
              <Send size={16} />
              {isSubmitting ? t('lawyerProposal.submitting') : t('lawyerProposal.submit')}
            </button>
          </div>
        </div>
      </div>
